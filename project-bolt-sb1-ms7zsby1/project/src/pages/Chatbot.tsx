  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-purple-900 to-pink-800">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-pink-200">Get personalized book and video recommendations</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="bg-white/10 backdrop-blur-lg rounded-2xl shadow-lg h-[600px] flex flex-col border border-white/20">
          {/* Messages */}
                ...
                <div className={`flex items-start space-x-3 max-w-3xl ${message.sender === 'user' ? 'flex-row-reverse space-x-reverse' : ''}`}>
                  ...
                  <div className={`rounded-2xl px-4 py-3 ${
                    ...
                    <p className={`text-xs mt-1 ${
                      message.sender === 'user' ? 'text-blue-100' : 'text-pink-200'
                    }`}>
            ...
            {messages[messages.length - 1]?.recommendations && (
              <div className="space-y-3">
                {messages[messages.length - 1].recommendations!.map((rec, index) => (
                  <div key={index} className="bg-white/20 backdrop-blur-lg border border-white/30 rounded-2xl p-4 shadow-sm hover:shadow-md transition-shadow duration-200">
                    <div className="flex items-start space-x-3">
                      <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                        rec.type === 'book' ? 'bg-blue-500/20' : 'bg-purple-500/20'
                      }`}>
                        {rec.type === 'book' ? (
                          <BookOpen className={`h-5 w-5 ${rec.type === 'book' ? 'text-blue-400' : 'text-purple-400'}`} />
                        ) : (
                          <Video className={`h-5 w-5 ${rec.type === 'book' ? 'text-blue-400' : 'text-purple-400'}`} />
                        )}
                      </div>
                      <div className="flex-1">
                        <div className="flex items-start justify-between">
                          <div>
                            ...
                            {rec.author && <p className="text-sm text-pink-200">by {rec.author}</p>}
                          </div>
                        ...
                        <p className="text-sm text-pink-100 mt-2">{rec.description}</p>
                        <button className="mt-3 text-orange-300 hover:text-orange-200 text-sm font-medium flex items-center space-x-1">
            ...
            {isTyping && (
              <div className="flex justify-start">
                <div className="flex items-start space-x-3 max-w-3xl">
                  ...
                  <div className="bg-white/20 backdrop-blur-lg rounded-2xl px-4 py-3 border border-white/30">
          ...
          {messages.length <= 1 && (
            <div className="px-6 py-3 border-t border-white/20">
              <p className="text-sm text-pink-200 mb-3">Quick questions to get started:</p>
              <div className="flex flex-wrap gap-2">
                {quickQuestions.map((question, index) => (
                  <button
                    key={index}
                    onClick={() => handleQuickQuestion(question)}
                    className="px-3 py-2 bg-white/20 backdrop-blur-lg text-white rounded-full text-sm hover:bg-white/30 transition-colors duration-200 border border-white/30"
                  >
                    {question}
                  </button>
                ))}
          ...
          <div className="p-6 border-t border-white/20">
            <div className="flex items-center space-x-3">
              <input
                ...
                className="flex-1 px-4 py-3 bg-white/20 backdrop-blur-lg border border-white/30 rounded-full text-white placeholder-white/70 focus:ring-2 focus:ring-orange-400 focus:border-transparent"
                disabled={isTyping}
              />
              <button
                onClick={handleSendMessage}
                disabled={!inputMessage.trim() || isTyping}
                className="w-12 h-12 bg-gradient-to-r from-orange-500 to-pink-500 text-white rounded-full flex items-center justify-center hover:from-orange-600 hover:to-pink-600 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed transform hover:scale-105"
              >
